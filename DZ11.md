...
(1 строка)
```
В Postgresql требуется намного больше и сложнее операций для хранения и последующего вывода данных. А так же запросы на соединение таблиц являются очень ресурсозатратными. Для обозначения объектов и связей между ними лучше использовать Neo4j.