...
Выдало ошибку, потому что по умолчанию стоит 127.0.0.1 для клиентских соединений, добавим в строку запуска экземплеров на всех ВМ --bind_ip_all (слушать со всех подключений) и перезапустим экземпляры. 
...
Ошибка появилась поскольку неявная проблема записи по умолчанию для этого сегмента установлена в {w : 1}, поскольку количество арбитров в конфигурации сегмента привело к увеличению количества число участников с правом голоса, доступных для записи, строго не должно превышать большинства голосующих. 

Необходимо установить ограничение на запись в масштабах кластера:
...
<pre><code>
[root@MongoSRD01 ~]# mongod --shardsvr --dbpath /home/mongo/sh1 --port 27011 --auth --bind_ip_all --replSet RS1 --fork --logpath /home/mongo/sh1/sh1.log --pidfilepath /home/mongo/sh1/sh1.pid
...
### Выводы: Во время выполнения домашнего задания, получилось построить шардированный кластер MongoDB потестировать работу отказоустойчивости, настроить аутентификацию и создать пользователей с ролями.